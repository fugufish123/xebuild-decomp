...
class ProjectDiffAnalyzer:
    def __init__(self):
        self.type_map = {
            r'\b__cdecl\b': '', r'\b__stdcall\b': '', r'\b__fastcall\b': '',
            r'\bundefined[1248]\b': 'int', r'\buint\b': 'unsigned int',
            r'\blonglong\b': 'long long', r'\bpointer\b': 'void*',
        }
        if not os.path.exists("diffs"):
            os.makedirs("diffs")

    def preprocess_source(self, code, target_func):
        """Standardizes C source for Clang analysis without penalizing FUN/DAT."""
        ...
        headers = [
            "#include <stdint.h>", "#include <string.h>", "#include <stddef.h>", "#include <stdbool.h>",
            "#ifndef _BYTE_DEFINED", "#define _BYTE_DEFINED", "typedef unsigned char byte;", "#endif",
    ...
    def analyze_ir(self, ir_text, func_name):
        """Extracts structural features (bigrams) to ensure sequence integrity."""
        if not ir_text: return None
        try:
    ...
    def validate_code_quality(self, s_raw, g_raw, fp_s, fp_g):
        """Checks for lazy copying and decompiler artifacts (excluding FUN/DAT)."""
        penalties = []
        multiplier = 1.0
    ...
    def generate_report(self, addr, f_name, g_raw, s_raw, fp_g, fp_s, penalties, score, src_path, exp_path, g_err, s_err):
        ...
        with open(path, "w", encoding="utf-8") as f:
            ...
            if penalties:
                ...
                for p in penalties: f.write(f"- {p}\n")
                f.write("\n")
    ...
    def run_analysis(self, exports_dir="exports", src_dir="src"):
        ...
        for file in sorted(os.listdir(exports_dir)):
            if file.endswith(".json") and not file.endswith(".cfg.json"):
                ...
                if not os.path.exists(src_c) or not os.path.exists(exp_c): continue

                with open(json_path, 'r') as f: meta = json.load(f)
                addr, name = meta.get('address', 'unk'), meta.get('name', base)

                with open(exp_c, 'r') as f: g_src = f.read()
                with open(src_c, 'r') as f: s_src = f.read()
                ...
                fp_g, fp_s = self.analyze_ir(ir_g, name), self.analyze_ir(ir_s, name)
                ...
                self.generate_report(addr, name, g_src, s_src, fp_g, fp_s, penalties, score, src_c, exp_c, err_g, err_s)
